...
let mongoSums = async function mongoSums(data)
{
    return new Promise(async function(resolve, reject)
    {
        ...
        for(var d in data){
            ...
            sums.assets++
            if(data[d].figures.price && data[d].figures.price > 10000 && data[d].figures.price < 1000000000){
                sums.price.sum = sums.price.sum + data[d].figures.price
                sums.price.count++
            }
            if(data[d].figures.rent && data[d].figures.rent > 100 && data[d].figures.rent < 1000000000){
                sums.rent.sum = sums.rent.sum + data[d].figures.rent
                sums.rent.count++
            }
            if(data[d].figures.price && data[d].figures.price > 10000 && data[d].figures.price < 1000000000
                && data[d].figures.rent && data[d].figures.rent > 100 && data[d].figures.rent < 1000000000
                && data[d].figures.yield && data[d].figures.yield > 2 && data[d].figures.yield < 30){
                sums.yield.sum = sums.yield.sum + data[d].figures.yield
                sums.yield.count++
            }
        }
